...
public class ReadOnlyModeTest extends ZKTestCase {
	...
	/**
	 * Basic test of read-only client functionality. Tries to read and write during
	 * read-only mode, then regains a quorum and tries to write again.
	 */
	@Test(timeout = 90000)
	...
	public void testConnectionEvents() throws Exception {
		...
		while (!(zk.getState() == States.CONNECTEDREADONLY)) {
			Thread.sleep(200);
			// FIXME this was originally 5 seconds, but realistically, on random/slow/virt
			// hosts, there is no way to guarantee this
	...
	/**
	 * Tests a situation when client firstly connects to a read-only server and then
	 * connects to a majority server. Transition should be transparent for the user.
	 */
	@Test(timeout = 90000)
	public void testSessionEstablishment() throws Exception {
		...
		// ZOOKEEPER-2722: wait until we can connect to a read-write server after the
		// quorum
		// is formed. Otherwise, it is possible that client first connects to a
		// read-only server,
		// then drops the connection because of shutting down of the read-only server
		// caused
		// by leader election / quorum forming between the read-only server and the
		// newly started
		// server. If we happen to execute the zk.create after the read-only server is
		// shutdown and
	...
	/**
	 * Ensures that client seeks for r/w servers while it's connected to r/o server.
	 */
