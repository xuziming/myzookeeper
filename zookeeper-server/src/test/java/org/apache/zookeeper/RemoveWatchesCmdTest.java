...
public class RemoveWatchesCmdTest extends ClientBase {
	...
	/**
	 * Test verifies default options. When there is no passed options, removewatches
	 * command will use default options - WatcherType.ANY and local=false
	 */
	@Test(timeout = 30000)
	public void testRemoveWatchesWithNoPassedOptions() throws Exception {
		...
		Assert.assertTrue("Failed to find child watches for the path testnode2",
				zk.getChildWatches().contains("/testnode2"));
	...
	public void testRemoveNodeCreatedWatches() throws Exception {
		...
		myWatcher1.matches();
		Assert.assertEquals("Failed to remove pre-create watches :" + zk.getExistWatches(), 1,
				zk.getExistWatches().size());
		Assert.assertTrue("Failed to remove pre-create watches :" + zk.getExistWatches(),
		...
		myWatcher2.matches();
		Assert.assertEquals("Failed to remove pre-create watches : " + zk.getExistWatches(), 0,
				zk.getExistWatches().size());
	}
	...
	public void testRemoveNodeDeletedWatches() throws Exception {
		...
		Assert.assertTrue("Failed to remove child watches :" + zk.getChildWatches(),
	...
	/**
	 * Test verifies deletion of watches locally when there is no server connection
	 */
	@Test(timeout = 30000)
	...
	private void verifyRemoveAnyWatches(boolean local) throws Exception {
		...
		Watcher watcher = new Watcher() {
			...
			private void addWatchNotifications(final Map<String, List<EventType>> pathVsEvent, WatchedEvent event) {
		...
		LOG.info("Remove watchers using shell command : {}", cmdstring);
		zkMain.cl.parseCommand(cmdstring);
		Assert.assertTrue("Removewatches cmd fails to remove child/data watches", zkMain.processZKCmd(zkMain.cl));
		LOG.info("Waiting for the WatchRemoved events");
		watcherLatch.await(10, TimeUnit.SECONDS);
		Assert.assertEquals("Didn't receives WatchRemoved events!", 1, pathVsEvent.size());
		Assert.assertTrue("Didn't receives DataWatchRemoved!",
				pathVsEvent.get("/testnode1").contains(EventType.DataWatchRemoved));
		Assert.assertTrue("Didn't receives ChildWatchRemoved!",
