...
public class RandomAccessFileReader extends Reader implements DataInput {
	...
	/**
	 * fill the buffer from the file. fp keeps track of the file pointer. fileoffset
	 * is the offset into the file to where the buffer came from.
	 */
	...
	/**
	 * copy what we can from buffer. if it's not enough, fill buffer again and copy
	 * again
	 */
	...
	synchronized public String readLine() throws IOException {
		...
		// go through buffer until i find a \n, if i reach end of buffer first, put
		// whats in buffer into string buffer,
		// repeat
		buffering: for (;;) {
			if (buffersize == 0) {
				fill();
